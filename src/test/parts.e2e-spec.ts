...
describe('Parts (e2e)', () => {
  ...
  describe('creation', () => {
    ...
    it('should reject a duplicated code (409)', async () => {
      await ctx.asAdmin('post', '/parts').send(PART_PAYLOAD).expect(409);
    });
  ...
  describe('reading', () => {
    ...
    it('should answer 404 for an unknown code', async () => {
      await ctx.asAdmin('get', '/parts/code/UNKNOWN-999').expect(404);
    });
  });
  ...
  describe('access control', () => {
    it('should block a mechanic on the parts (403)', async () => {
      await ctx.asMechanic('get', '/parts').expect(403);
    });

    it('should block an anonymous request on the parts (401)', async () => {
      await ctx.anonymous('get', '/parts').expect(401);
    });
  });
});
