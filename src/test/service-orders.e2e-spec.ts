...
describe('ServiceOrders (e2e)', () => {
  ...
  describe('opening', () => {
    ...
    it('should reject an unknown vehicle (404)', async () => {
      const { status } = await ctx.asMechanic('post', '/service-orders').send({
        customerId,
  ...
  describe('access control', () => {
    it('should let a mechanic list the orders', async () => {
      const { status } = await ctx.asMechanic('get', '/service-orders');
      expect(status).toBe(200);
    });

    it('should let an admin list the orders', async () => {
      const { status } = await ctx.asAdmin('get', '/service-orders');
      expect(status).toBe(200);
    });
    ...
    it.each([
      ...
      async (method, url) => {
        const { status } = await ctx.anonymous(method as 'get' | 'post', url);
