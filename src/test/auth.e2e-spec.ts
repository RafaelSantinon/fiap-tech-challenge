...
describe('Auth (e2e)', () => {
  ...
  describe('login', () => {
    ...
    it('should reject login with an invalid password (401)', async () => {
      await login({ email: ADMIN.email, password: 'ErrandoASenha@1' }).expect(
        401,
      );
    });
  ...
  describe('current user', () => {
    ...
    it('should block /auth/me without a token (401)', async () => {
      await ctx.anonymous('get', '/auth/me').expect(401);
    });
  });
  ...
  describe('logout', () => {
    ...
    it('should block logout without a token (401)', async () => {
      await ctx.anonymous('post', '/auth/logout').expect(401);
    });
  });
});
