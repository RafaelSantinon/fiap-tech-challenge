...
describe('Quotes (e2e)', () => {
  ...
  describe('reading', () => {
    ...
    it('should answer 404 for an unknown quote', async () => {
      await ctx
        .asAdmin('get', '/quotes/00000000-0000-4000-8000-000000000000')
        .expect(404);
    });

    it('should reject an id that is not a uuid (400)', async () => {
      await ctx.asAdmin('get', '/quotes/nao-e-uuid').expect(400);
    });
  });
  ...
  describe('filters', () => {
    ...
    it('should reject an invalid status filter (400)', async () => {
      await ctx.asAdmin('get', '/quotes?status=aguardando').expect(400);
    });
  });

  describe('access control', () => {
    it('should let both admin and mechanic read the quotes', async () => {
      await ctx.asAdmin('get', '/quotes').expect(200);
      await ctx.asMechanic('get', '/quotes').expect(200);
    });

    it('should block an anonymous request on the quotes (401)', async () => {
      await ctx.anonymous('get', '/quotes').expect(401);
      await ctx.anonymous('get', `/quotes/${quoteId}`).expect(401);
    });
  });
});
