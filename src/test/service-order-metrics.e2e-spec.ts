...
describe('ServiceOrderMetrics (e2e)', () => {
  ...
  describe('access control', () => {
    it('should block a mechanic on both metrics (403)', async () => {
      const byStatus = await ctx.asMechanic('get', STATUS_ROUTE);
      expect(byStatus.status).toBe(403);

      const byService = await ctx.asMechanic('get', SERVICE_ROUTE);
      expect(byService.status).toBe(403);
    });

    it('should block an anonymous request on both metrics (401)', async () => {
      const byStatus = await ctx.anonymous('get', STATUS_ROUTE);
      expect(byStatus.status).toBe(401);

      const byService = await ctx.anonymous('get', SERVICE_ROUTE);
      expect(byService.status).toBe(401);
    });
  });
});
